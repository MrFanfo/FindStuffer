...
export function ProjectsView({ categories, locations, onBack, onOpenItem }: { categories: Category[]; locations: LocationNode[]; onBack: () => void; onOpenItem: (item: Item) => void }) {
  ...
  const requirementGroups = new Map<number | null, { name: string; entries: Requirement[] }>();
  for (const entry of project?.requirements || []) {
    ...
    if (!requirementGroups.has(key)) requirementGroups.set(key, { name: category?.path || category?.name || 'Uncategorised', entries: [] });
    requirementGroups.get(key)!.entries.push(entry);
  }
  const sortedGroups = [...requirementGroups].sort(([a, first], [b, second]) => a === null ? 1 : b === null ? -1 : first.name.localeCompare(second.name));
