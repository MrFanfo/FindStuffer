...
import { CategoryMarkTools } from "../places/CategoryMarkTools";
...
export function DataView({ categories, locations, busy, offline, onBack, onChanged, setNotice }: {
  ...
  return <section className="workspace-page data-page">
    ...
    <section className="workspace-card data-marks-section"><header><span><Icon name="tag" size={22} /></span><div><h2 className="eyebrow">CATEGORY MARKS</h2></div></header><p>Fill in marks for categories that have none, or save and restore the whole set. Marks chosen by hand are never replaced. Change one category's mark from its row in Places → Categories.</p><CategoryMarkTools busy={busy} onChanged={() => void onChanged()} /></section>
  </section>;
}
