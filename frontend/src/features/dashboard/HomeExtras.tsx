import { recentlyViewed } from "./recentItems";
...
export function HomeExtras({ locations, onItem, onPlace, onInbox, onManage, onCreatePlace, onCapture, onPrint, empty }: {
  ...
  const [viewed] = useState(recentlyViewed);
  const [recent, setRecent] = useState<Item[]>([]);
  const [error, setError] = useState("");
  const [picker, setPicker] = useState(false);
  const places = flattenLocations(locations);
  const load = async () => {
    const results = await Promise.allSettled([api.attention(), api.inventoryQuery("", {}, true)]);
    if (results[0].status === 'fulfilled') setAttention(results[0].value);
    if (results[1].status === 'fulfilled') setRecent(results[1].value.items.slice(0, 5));
    setError(results.some((result) => result.status === 'rejected') ? 'Some Home sections could not load.' : '');
  };
  useEffect(() => { void load(); }, []);
  ...
  return <>
    ...
    {viewed.length > 0 && <section className="home-section"><h2>Recently viewed</h2><div className="recent-items">{viewed.map((item) => <button key={item.public_id} onClick={() => void open(item.public_id)}><strong>{item.name}</strong><small>{item.location_path}</small></button>)}</div></section>}
    {recent.length > 0 && <section className="home-section"><h2>Recently updated</h2><div className="recent-items">{recent.map((item) => <button key={item.public_id} onClick={() => onItem(item)}><strong>{item.name}</strong><small>{item.location_path} · {item.quantity} {item.unit}</small></button>)}</div></section>}
