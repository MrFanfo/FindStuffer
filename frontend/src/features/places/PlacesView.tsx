 ...
 */
async function downloadEnrichment(scope: { category?: number; place?: string }, name: string): Promise<void> {
  const payload = await api.enrichmentExport(scope);
...
export function LocationDetailView({ locationId, locations, categories, locationTypes, busy, onQueuePrint, onOpenItem, onOpenLocation, onAddHere, onPutAwayHere, onCreateLocationHere, onDefaultsChanged, onBack }: {
  locationId: string;
  locations: LocationNode[];
  categories: Category[];
  locationTypes: LocationType[];
