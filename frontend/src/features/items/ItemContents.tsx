...
export function ItemContents({ item, editing, locations, categories, onChanged }: { item: Item; editing: boolean; locations: LocationNode[]; categories: Category[]; onChanged: (item: Item) => Promise<void> }) {
  ...
  return <section className="detail-section item-contents"><div className="section-heading"><h2>{item.is_container ? 'Contents' : 'Storage'}</h2>{editing && <label><input type="checkbox" checked={Boolean(item.is_container)} disabled={busy} onChange={event => void perform(() => api.updateItem(item, { is_container: event.target.checked }))} /> Can contain items</label>}</div>
